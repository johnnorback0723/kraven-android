...
class ToppingBottomSheetBeverageEditFragment : DialogFragment() {
    private var count = 0
    private var beverage: DishesItem? = null
    private var type: String? = null
    private var mParentListener: OnChildFragmentInteractionListenersEdit? = null
    ...
    override fun onAttach(context: Context) {
        ...
        if (context is IsolatedActivity) {
            ...
            val parentFragment = mainActivity.getCurrentFragment<CartFragment>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (arguments != null) {
            beverage = arguments?.getParcelable("NewDishList")
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        view.textViewCount.text = beverage?.qty.toString()
        view.textViewSpecialNote.setText(beverage?.specialNote)

        count = beverage?.qty!!
        finalCount = beverage?.qty!!
       ...
       /* val foodPrice = if (beverage!!.specialOffer != null && beverage!!.specialOffer == 0F) {
            beverage!!.price!!
        } else {
            beverage!!.originalPrice!!.toFloat()
        }*/
        ...
        if (beverage?.toppings != null) {
            ...
            toppingTitleAdapter!!.items = beverage!!.toppings
        }
        ...
        view.textViewFoodPrice.text = if (beverage?.specialOffer != 0F)
            convertTwoDigit(foodPrice)
        else convertTwoDigit(beverage?.price!!)
        ...
        if (view.textViewOffPrice.visibility == View.VISIBLE) {
            ...
            view.textViewOffPrice.text = convertTwoDigit(beverage?.price!!)
        }*/
        ...
        view.buttonAddToCart.setOnClickListener {
            ...
            if (firstCheckIs != null) {
                ...
                dishesItem.originalPrice = beverage!!.originalPrice
                //dishesItem.vat = beverage!!.vat
                dishesItem.vatPrice = beverage?.vatPrice
                ...
                dishesItem.image = beverage?.image
                dishesItem.status = beverage?.status
                dishesItem.milliliter = beverage?.milliliter
                dishesItem.specialOffer = beverage?.specialOffer
                dishesItem.name = beverage?.name.toString()
              ...
              /*  val foodPrices = if (beverage!!.specialOffer != null && beverage?.specialOffer == 0F) {
                    beverage!!.price
                } else {
                    beverage!!.originalPrice!!.toFloat()
                }*/
                ...
                mParentListener?.addBeverage(dishesItem.totalPrice!!, dishesItem, ConstantApp.PassValue.ORDER_BEVERAGE)
                dismiss()
    ...
    interface OnChildFragmentInteractionListenersEdit {
        fun addBeverage(finalPrice: Float, items: DishesItem, orderPage: String)
    }
