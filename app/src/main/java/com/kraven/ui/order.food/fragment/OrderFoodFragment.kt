...
class OrderFoodFragment : BaseFragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        homeViewModel.getRestaurantList.observe(this, {
            showLoader(false)
            when (it.code) {
        ...
        homeViewModel.getBeverageList.observe(this, { it ->
            ...
            when (it.code) {
                StatusCode.CODE_SUCCESS -> {
                    ...
                    restaurantAdapter?.setItems(sortedList, page)
                }
                StatusCode.CODE_NO_DATA -> {
                    if (page == 1) {
                        restaurantAdapter?.clearAllItem()
    ...
    private fun callBeverageList(page: Int) {
    ...
    private fun callRestaurantList(page: Int) {
        val parameters = Parameters()
        parameters.latitude = homeViewModel.currentLatLng?.latitude.toString()
        parameters.longitude = homeViewModel.currentLatLng?.longitude.toString()
    ...
    private fun setUpRecyclerView() {
        ...
        restaurantAdapter = RestaurantAdapter(true,object : RestaurantAdapter.ItemClickListener {
            ...
            override fun onItemClick(item: Restaurants) {
                ...
                if (item.distanceRadius!! > 0) {
                    ...
                    if (item.mainBranchId==null || item.mainBranchId == "0") {
                        val bundle = Bundle()
                        bundle.putString(ConstantApp.PassValue.RESTAURANT_ID, item.id.toString())
                        bundle.putParcelable(ConstantApp.PassValue.CURRENT_LAT_LONG, homeViewModel.currentLatLng)
                        bundle.putString(ConstantApp.PassValue.RESTAURANT_DISTANCE, item.distance.toString())
                        ...
                        navigator.load(SubBranchFragment::class.java)
                                .setBundle(bundleOf(ConstantApp.PassValue.ITEM to item,
                                        ConstantApp.PassValue.ORDER_FOOD to orderPage))
                                .replace(true)
                    }
        ...
        endlessRecyclerViewScrollListener = object : EndlessRecyclerViewScrollListener(linearLayoutManager) {
            override fun onLoadMore(page: Int, totalItemsCount: Int, view: RecyclerView?) {
                this@OrderFoodFragment.page = page
                Handler().postDelayed({
        ...
        recyclerViewOrderFood.addOnScrollListener(endlessRecyclerViewScrollListener!!)
    }
